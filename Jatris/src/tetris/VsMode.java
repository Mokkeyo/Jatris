...
/**
 * Ein Modus wo ein Spieler gegen ein anderen Spieler in Tetris spielt (nicht fertig)
 * 
 * @author Umut-Can.Bektas
 */
public class VsMode extends JPanel {
	final int WIDTH = 360;
	...
	public VsMode() {
		...
		left_x = (Panel.WIDTH / 3) - (WIDTH / 2);
		right_x = left_x + WIDTH + 40;
		top_y = 50;
		...
		P2_MINO_START_X = right_x + (WIDTH / 2) - Block.SIZE;
		P2_MINO_START_Y = top_y + Block.SIZE;

		// spawns the block at the Position
//		ChangeCurrentMinoPosition();
		p2CurrentMino = pickMino();
		p2CurrentMino.setXY(P2_MINO_START_X, P2_MINO_START_Y);
		...
		p2NextMino = pickMino();
		p2NextMino.setXY(P2_NEXTMINO_X, P2_NEXTMINO_Y);
	}
...
//					}
		...
		if (lineCount > 0) {
			System.out.println("Add blocks to enemy");
		}
	}
...
//		HOLDMINO_Y = top_y + 110;
	...
	public void draw(Graphics2D g2D) {
		...
		g2D.setStroke(new BasicStroke(4f));
		g2D.drawRect(right_x - 4, top_y - 4, WIDTH + 8, HEIGHT + 8);

		// Draws the Box to show which block is next for Player 1
		int x = right_x - 590;
		int y = bottom_y - 600;
		...
		g2D.drawString("NEXT", x + 35, y + 60);

		// Draw Hold for Player 1
		int x2 = right_x - 590;
		int y2 = bottom_y - 430;
		g2D.setColor(Color.red);
		g2D.drawRect(x2, y2, 150, 150);
		g2D.drawString("HOLD", x2 + 35, y2 + 60);

		// Draws the Box to show which block is next for Player 2
		x = right_x + 400;
		y = bottom_y - 600;
		g2D.setColor(Color.orange);
		g2D.drawRect(x, y, 150, 150);
		...
		g2D.drawLine(HEIGHT, WIDTH * 2, HEIGHT, 2);

		// Draw currentMino
		if (currentMino != null) {
			currentMino.draw(g2D);
		}
		if (p2CurrentMino != null) {
			p2CurrentMino.draw(g2D);
		}
